...
vec3 Temperature(float t)
{
    ...
    if (t < 0.5) return mix(blue, cyan, t * 2.0);
...
vec4 GetColor(float t)
{
	switch (u_UBO.Color)
	{
		case 0:		return vec4(Temperature(t), 1.0);
		case 1:		return vec4(Ocean(t), 1.0);
		case 2:		return vec4(Fire(t), 1.0);
		case 3:		return vec4(BlackAndWhite(t), 1.0);
	}
}
